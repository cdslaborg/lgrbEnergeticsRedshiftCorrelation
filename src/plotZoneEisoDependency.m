...
... threshMask = eiso > exp( getLogThreshLim( log(zone) ) );
threshMask = liso > exp( getLogThreshLim( log(zone) ) );

refinedZone = zone(threshMask); % select only those events above the threshold line
... refinedEiso = eiso(threshMask); % select only those events above the threshold line
refinedLiso = liso(threshMask);

logRefinedZone = log(zone);
... logRefinedEiso = log(eiso);
...
... stat = getZoneEisoDependency(logRefinedZone, logRefinedEiso, logRefinedZoneMax);
