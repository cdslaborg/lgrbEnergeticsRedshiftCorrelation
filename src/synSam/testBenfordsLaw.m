...
synY = zeros(1,length(synSam.yint));
for i = 1:length(synSam.yint)
    a = char(string(synSam.yint(i)));
    if a(1) == '0'
        synY(i) = a(3);
    else
        synY(i) = a(1);
...
figure("color", figureColor); hold on; box on;
    ...
    legend([p,p1,p3,p2]...
        ...
        ,"\tau = " + string(round(averageValues.avgTau,2)) + " at 50% detection limit" ...
...
figure("color", figureColor); hold on; box on;
    ...
    legend([p,p1,p2,p3]...
            ,"detection limit at 50%-probability" ...
            , "\alpha = " + string(round(averageValues.avgAlpha,2)) + alphaStdString + " at 50% detection limit"...
            , "detection limit at 99%-probability (compareable to " + citedWorks + ")"...
